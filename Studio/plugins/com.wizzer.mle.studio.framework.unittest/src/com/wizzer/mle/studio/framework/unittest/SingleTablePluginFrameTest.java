...
/**
 * This class is a unit test for com.wizzer.mle.studio.framework.ui.SingleTablePluginFrame.
 * 
 * @author Mark S. Millard
 */
public class SingleTablePluginFrameTest extends TestCase
{
	/**
	 * Constructor for SingleTablePluginFrameTest.
	 * 
	 * @param name
	 */
	public SingleTablePluginFrameTest(String name)
	{
		super(name);
	}
	...
	{
		junit.textui.TestRunner.run(SingleTablePluginFrameTest.class);
	}
	...
	/**
	 * Test the <code>SingleTablePluginFrame</code>.
	 */
	public void testSingleTablePluginFrame()
	{
		class MyTable extends Table
		{
			class MyTreeNodeAttribute extends VariableListAttribute implements Observer
			{
				...
				{ super(menuName,countAttribute,functionName,theCaller,null); }
		...
		class MySingleTablePluginFrame extends SingleTablePluginFrame
		{
			public MySingleTablePluginFrame(Composite parent, String mainTitle, Table table)
			{ super(parent,mainTitle,table); }
			
			protected void resetTable(Table table) {}  // Do nothing for now.
			...
			public void addActions(Composite parent) {};  // Do nothing for now.

			public void handleNetworkSendAction(Event e) {};  // Do nothing for now.
			public void handleNetworkStopSendingAction(Event e) {};  // Do nothing for now.
			public void handleNetworkUpdateAction(Event e) {};  // Do nothing for now.
			public void handleViewConsoleAction(Event e) {};  // Do nothing for now.
			public void handleViewDebugConsoleAction(Event e) {};  // Do nothing for now.
		}
		
		Shell shell = new Shell();
		shell.setText("SingleTablePluginFrame Unit Test");
		...
		Composite composite = new Composite(shell, SWT.RESIZE);
		composite.setLayout(new FillLayout());
		
		final MySingleTablePluginFrame pluginUnitTest = new MySingleTablePluginFrame(composite,
			"SingleTablePluginFrame Unit Test",domainTable);
...
*/
		...
		while (!shell.isDisposed()) {
			if (!display.readAndDispatch()) {
				display.sleep();
				pluginUnitTest.processMessages();
			}
		}
	}
