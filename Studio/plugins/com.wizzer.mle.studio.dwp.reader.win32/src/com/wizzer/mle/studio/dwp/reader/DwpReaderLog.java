...
// Declare package.
package com.wizzer.mle.studio.dwp.reader;
 ...
 * Magic Lantern Studio DwpReader Win32 plug-in fragment.
 * <p>
 ...
 */
public class DwpReaderLog implements ILog
{
	// A singleton instance of the Magic Lantern Studio DWP log.
	private static DwpReaderLog m_log = null;
	...
	{
		if (DwpPlugin.getDefault() != null)
			DwpPlugin.getDefault().getLog().log(
				new Status(Status.ERROR, DwpPlugin.getID(), Status.OK, message, ex));
	}
	...
	private static void logEclipseWarning(String message)
	{
		if (DwpPlugin.getDefault() != null)
			DwpPlugin.getDefault().getLog().log(
				new Status(Status.WARNING, DwpPlugin.getID(), Status.OK, message, null));
	}
	...
	private static void logEclipseInfo(String message)
	{
		if (DwpPlugin.getDefault() != null)
			DwpPlugin.getDefault().getLog().log(
				new Status(Status.INFO, DwpPlugin.getID(), Status.OK, message, null));
	}
	
	/**
	 * Get the singleton instance of the Magic Lantern Studio DwpReader Log. If the instance does not yet
	 * exist, it will be created.
	 * 
	 * @return A reference to a <code>DwpReaderLog</code> is returned.
	 */
	public static DwpReaderLog getInstance()
	{
		if (m_log == null)
			m_log = new DwpReaderLog();
		return m_log;
	}
	...
	/**
	 * Log an error to the global Magic Lantern Studio DWP log.
	 * <p>
	 * The message will be logged to <code>System.err</code> as well as the Eclipse log file.
	 * This method will also print a stack trace for the specified exception.
	 * </p>
	...
	{
		DwpReaderLog log = getInstance();
		log.error(ex,message);
	}

	/**
	 * Log a warning message to the global Magic Lantern Studio DWP log.
	 * <p>
	 * The message will be logged to <code>System.out</code> as well as the Eclipse log file.
	 * </p>
	 * 
	...
	/**
	 * Log an informative message to the global Magic Lantern Studio DWP log.
	 * <p>
	 * The message will be logged to <code>System.out</code> as well as the Eclipse log file.
	 * </p>
	 * 
