 ...
 * Magic Lantern Studio Framework plug-in.
 * <p>
 ...
 * @created Nov 6, 2003
 */
public class FrameworkLog implements ILog
{
	// A singlton instance of the Studio Framework log.
	private static FrameworkLog m_log = null;
	...
	private static void logEclipseError(Exception ex, String message)
	{
		if (FrameworkPlugin.getDefault() != null)
			FrameworkPlugin.getDefault().getLog().log(
				new Status(Status.ERROR, FrameworkPlugin.getID(), Status.OK, message, ex));
	}
	...
	private static void logEclipseWarning(String message)
	{
		if (FrameworkPlugin.getDefault() != null)
			FrameworkPlugin.getDefault().getLog().log(
				new Status(Status.WARNING, FrameworkPlugin.getID(), Status.OK, message, null));
	}
	...
	private static void logEclipseInfo(String message)
	{
		if (FrameworkPlugin.getDefault() != null)
			FrameworkPlugin.getDefault().getLog().log(
				new Status(Status.INFO, FrameworkPlugin.getID(), Status.OK, message, null));
	}
	
	/**
	 * Get the singleton instance of the Studio Framework Log. If the instance does not yet
	 * exist, it will be created.
	 * 
	 * @return A reference to a <code>FrameworkLog</code> is returned.
	 */
	public static FrameworkLog getInstance()
	{
		if (m_log == null)
			m_log = new FrameworkLog();
		return m_log;
	}
	...
	/**
	 * Log an error to the global Studio Framework log.
	 * <p>
	 * The message will be logged to <code>System.err</code> as well as the Eclipse log file.
	 * This method will also print a stack trace for the specified exception.
	 * </p>
	...
	{
		FrameworkLog log = getInstance();
		log.error(ex,message);
	}

	/**
	 * Log a warning message to the global Studio Framework log.
	 * <p>
	 * The message will be logged to <code>System.out</code> as well as the Eclipse log file.
	 * </p>
	 * 
	...
	/**
	 * Log an informative message to the global Studio Framework log.
	 * <p>
	 * The message will be logged to <code>System.out</code> as well as the Eclipse log file.
	 * </p>
	 * 
