 ...
 * <i>genscene</i> on resources that are Digital Workprints.
 * 
 * @author Mark S. Millard
 */
public class GensceneBuildVisitor implements IResourceVisitor
{
     ...
     */
    public GensceneBuildVisitor(IProgressMonitor monitor)
    {
        super();
        m_monitor = monitor;
    }
    ...
    /**
     * Visits the given resource and runs <b>genscene</b> on it if it is a
     * Digital Workprint.
     * 
    ...
    public boolean visit(IResource resource) throws CoreException
    {
        ...
        try
        {
	        	...
	        	(resource.getFileExtension().equals("dwp")))
	        {
	            if (DwpUtilities.isDigitalWorkprint((IFile)resource))
	            {
	                ...
	                {
	                    // Begin monitoring.
	                    m_monitor.beginTask("Executing genscene.", 3);
	                    
	                    // Create a genscene command.
		                m_command = new GensceneBuildCommand(resource).getCommand();
        ...
        {
            DppLog.logError(ex,"Unable to run genscene on " + resource.getName() + ".");
        }
    ...
    private void setDerivedAssets(IResource dwp) throws DppException
    {
        ...
        try
        {
	        ...
	        // Process genscene assets.
	        String sceneidFile = GenscenePropertyManager.getInstance().getSceneIdValue(dwp);
