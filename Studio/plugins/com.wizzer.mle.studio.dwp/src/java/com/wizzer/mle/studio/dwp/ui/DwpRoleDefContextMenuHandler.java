/*
 * DwpRoleDefContextMenuHandler.java
 ...
 * <code>AttributetreeViewer</code> for a <code>DwpRoleDefAttribute</code>.
 *
 * @author Mark S. Millard
 */
public class DwpRoleDefContextMenuHandler extends AbstractItemContextMenuHandler
{
     ...
     */
    public DwpRoleDefContextMenuHandler()
    {
        super();
    }
	...
	{
		DwpRoleDefAttribute setDef = (DwpRoleDefAttribute)m_viewer.getSelectedAttribute();
		
		if (type == DwpItemAttribute.TYPE_DWP_PROPERTYDEF)
		{
    ...
    public Menu getContextMenu(IAttribute attribute, AttributeTreeViewer viewer)
    {
		...
		if (type == DwpRoleDefAttribute.TYPE_DWP_ROLEDEF)
		{
			init(viewer);
			
			Attribute packageAttr = ((DwpRoleDefAttribute)attribute).getChildByName("Package");
			if (packageAttr != null)
			{
