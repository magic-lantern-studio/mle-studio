...
class SdkSelectionPage extends WizardPage implements ITargetChangeListener {
    ...
    private void loadSamplesForTarget(IAndroidTarget target) {
        ...
        if (target != null) {
        	String samplesRootPath;
        	if (mValues.mleTarget != null) {
        		// Get samples for Magic Lantern.
        		samplesRootPath = mValues.mleTarget.getPath(target, MleTargetState.SAMPLES);
        		if (samplesRootPath == null)
        			return;
        	} else {
	            // Get the sample root path and recompute the list of samples
	            samplesRootPath = target.getPath(IAndroidTarget.SAMPLES);
        	}
